(function (root, factory) {
  ...
  function getTargetProductId(browserRoot = root) {
    try {
      return browserRoot && browserRoot.location
        ? new URLSearchParams(browserRoot.location.search || '').get('produto') || ''
        : '';
    } catch (_) { return ''; }
  }
  ...
  function selectFeaturedProductsForView(products, targetProductId, limit = 4) {
    ...
    return [...list].sort((a, b) => Number(a.ordem || 0) - Number(b.ordem || 0)).slice(0, limit);
  }

  function selectFeaturedProducts(products, limit = 4) {
    return selectFeaturedProductsForView(products, getTargetProductId(), limit);
  }
  ...
  function createFallbackSearch(browserRoot) {
    ...
    return {
      filterProducts(products, searchTerm, category) {
        const list = Array.isArray(products) ? products : [];
        const requestedId = getTargetProductId(browserRoot);
        const term = normalize(searchTerm);
        const selected = category || 'all';
        return list.filter(item => {
          if (!item) return false;
          if (requestedId && String(item.id) === String(requestedId)) return false;
          if (selected !== 'all' && item.categoria !== selected) return false;
          if (!term) return true;
          return normalize([item.nome, item.categoria, item.categoria_label].join(' ')).includes(term);
        });
      }
    };
  }
  ...
  return { selectFeaturedProducts, selectFeaturedProductsForView, getNextSlideIndex, getTargetProductId, createFallbackSearch };
});
