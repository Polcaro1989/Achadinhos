(function (root, factory) {
  ...
  function createFallbackSearch(browserRoot) {
    ...
    function targetId() {
      try {
        if (!browserRoot || !browserRoot.location) return '';
        return new URLSearchParams(browserRoot.location.search || '').get('produto') || '';
      } catch (_) { return ''; }
    }
  ...
  return { selectFeaturedProducts, getNextSlideIndex, createFallbackSearch };
});
