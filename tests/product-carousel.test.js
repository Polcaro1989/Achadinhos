...
test('seleciona no máximo quatro produtos pela prioridade de ordem', async () => {
  ...
  const products = [{ id: 5, ordem: 5 }, { id: 2, ordem: 2 }, { id: 1, ordem: 1 }, { id: 4, ordem: 4 }, { id: 3, ordem: 3 }];
...
test('fallback remove da grade o produto que ja esta no banner', async () => {
  ...
  assert.deepEqual(search.filterProducts(products, '', 'all'), [products[0]]);
});
